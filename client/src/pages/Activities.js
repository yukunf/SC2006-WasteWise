...
const Activities = () => {
    ...
    useEffect(() => {
        if (ratingData && companies.length > 0 && reportData) {
            const combinedData = [];
    ...
    }, [ratingData, companies, reportData]);
    ...
    return (
        <div className="w-full h-full">
            ...
            <div className="pl-10 lg:ml-20 pr-10 pb-10 w-[90%] overflow-x-auto lg:overflow-visible">
                <table className="table-auto text-left bg-white rounded-3xl font-poppins lg:mt-[-250px] ml-auto mr-auto shadow-2xl">
                    <thead>
                        <tr>
                            ...
                            <th className="p-4 text-[#016a70] font-bold text-base text-center border-b border-r border-[#937070]">REMARKS</th>
                        </tr>
                    </thead>
                        ...
                        <tbody>
                            {currentActivities.map((activity, index) => (
                                <tr key={index}>
                                    ...
                                    <td className={`p-4 text-black font-semibold w-[40%] border-r border-[#937070] ${index === currentActivities.length - 1 ? "border-b-0" : "border-b"}`}>
                                        {activity.content}
                                        <p className="font-light italic">
                                            {activity.activity_type === 'Reporting Collector' ? 
                                            <p>Reason: {activity.reason}<br />Comments: {activity.comments}</p> 
                                            : activity.activity_type === 'Rating Collector' ? 
                                            <p>Rating: {activity.rating}/5<br />Comments: {activity.comments}</p>  
                                            : 'N/A'}
                                        </p>
                                    </td>
